...
namespace TicTacToeWinFormApp
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void buttonPosition_Click(object sender, EventArgs e)
        {
            ...
            GameStatus gamestatus = CheckGameStatus(currentPlayerNumber);
            //sprawdzenie czy koniec(win or draw)
            if (gamestatus == GameStatus.Win)
            {
                MessageBox.Show("Wugrana " + players[currentPlayerNumber] + " !!!!!");
            ...
            currentPlayerNumber = (currentPlayerNumber == 0) ? 1 : 0; //warunek 
        }

        private GameStatus CheckGameStatus(int currentPlayerNumber)
        {
            for(int col = 0; col < 3; col++)
            {
